...
//console.log('in fromColumnValue, options.timezone = ' + options.timezone); // BB TEMP options undefined here
console.log('in fromColumnValue, this.client.config.connectionConfig.timezone = ' + this.client.config.connectionConfig.timezone); // BB TEMP - Seems to Read from datasources
console.log('in fromColumnValue, this.settings.timezone = ' + this.settings.timezone); // BB TEMP
  if (val == null) {
    return val;
  }
  if (prop) {
	...
	var originalString = dateString;
        if (prop.dataType == 'date' || (prop.mysql && prop.mysql.dataType == 'date')) { // BB TEMP This test from @darknos, prop.dataType = undefined, prop.mysql = [object Object], prop.mysql.dataType = date
console.log('in fromColumnValue, prop.dataType = '+prop.dataType); //BB TEMP
console.log('in fromColumnValue, prop.mysql = '+prop.mysql); //BB TEMP
console.log('in fromColumnValue, prop.mysql.dataType = '+prop.mysql.dataType); //BB TEMP
	  dateString += ' 00:00:00';
        }
        if (this.settings.timezone !== 'local') {
          dateString += ' ' + this.settings.timezone;
        }
