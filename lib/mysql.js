...
MySQL.prototype.connect = function(callback) {
  ...
  if (this.client) {
    ...
    this.client.getConnection(function(err, connection) {
      ...
      if (!err) {
        if (self.debug) {
          debug(g.f('MySQL connection is established: %s', self.settings || {}));
        }          
        connection.release();
      } else {
        if (self.debug || !callback) {
          console.error(g.f('MySQL connection is failed: %s %s', self.settings || {}, err));
        }
      }
...
MySQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (typeof callback !== 'function') {
    throw new Error(g.t('callback should be a function'));
  }
  if (debugEnabled) {
    debug(g.f('SQL: %s, params: %j', sql, params));
  }
  ...
  function runQuery(connection, release) {
    connection.query(sql, params, function(err, data) {
      if (debugEnabled) {
        if (err) {
          debug(g.f('Error: %j', err));
        }
        debug(g.f('Data: %s', data));
      }
  ...
  if (transaction && transaction.connection &&
    ...
    if (debugEnabled) {
      debug(g.t('Execute SQL within a transaction'));
    }
