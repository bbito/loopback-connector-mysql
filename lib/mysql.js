...
MySQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date) {
    //bbito - adding logs
	  console.log("toColumnValue: val instanceof Date: " + val instanceof Date); //bbito - evaluates to false, see prototype test below
	  console.log("toColumnValue: Date, val = " + val);
	  console.log("toColumnValue: val.toJSON" + val.toJSON());
	  if (Object.prototype.toString.call(val) === '[object Date]'){
	  console.log("toColumnValue: Object.prototype.toString.call(val) === '[object Date]' = TRUE");
	  }
    // bbito - commenting OUT this line: 
    // val = new Date(val); // bbito - commenting OUT this line
    return val;
  }
...
MySQL.prototype.fromColumnValue = function(prop, val) {
  ...
  if (prop) {
    switch (prop.type.name) {
      ...
      case 'Date':
      //bbito - adding logs
	  console.log("fromColumnValue: val instanceof Date: " + val instanceof Date); //bbito - evaluates to false, see prototype test below
	  console.log("fromColumnValue Date, val = " + val);
	  console.log("fromColumnValue: val.toJSON" + val.toJSON());
	  if (Object.prototype.toString.call(val) === '[object Date]'){
	  console.log("fromColumnValue: Object.prototype.toString.call(val) === '[object Date]' = TRUE");
	  }
        // bbito - commenting OUT this line: 
		//val = new Date(val.toString().replace(/GMT.*$/, 'GMT'));
		//val = new Date(val)); //bbito - Error! wrapping val in new Date crashes with "WARNING: LoopBack connector "mysql" is not installed..."
        break;
      case 'Boolean':
        val = Boolean(val);
        break;
